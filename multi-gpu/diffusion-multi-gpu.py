...
SAVE_DIR = "./diff-multi-gpu-training/"
LOAD_PATH = "./diff-multi-gpu-training/weight_epoch600.pt"
# LOAD_PATH = None
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.cond:
            return {"audio": audio, "spectrogram": spectrogram.T}
...
class Collator:
    ...
    def collate(self, minibatch):
        ...
        for record in minibatch:
            if not self.cond:
                ...
                start = random.randint(
                    0, record["spectrogram"].shape[0] - CROP_MEL_FRAMES
                )
        ...
        if not self.cond:
            return {"audio": torch.from_numpy(audio), "spectrogram": None}
...
class DDPM(nn.Module):
    ...
    def generate(self, spectrogram):
        ...
        x = torch.randn(
            spectrogram.shape[0], 256 * spectrogram.shape[-1], device=self.device
        )
        ...
        for epoch in tqdm(
            ...
            for i, audio_data in enumerate(self.dataloader):
                ...
                predict_noise = self.model(
                    noised_audio, torch.tensor(t, device=self.device), spectrogram
                ).squeeze()
...
if __name__ == "__main__":
    ...
    trainer = Trainer(
        model, train_data, SAVE_DIR, EPOCHS, SAVE_PER_EPOCH, DDPM, LOAD_PATH
    )
    trainer.train()
