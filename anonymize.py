...
import numpy as np
from diffusion_contentvec import DiffWave, DDPM, ContentVecExtractor, Trainer, Dataset, Collator
...
class Anonymizer:
    def __init__(self, diff_model_path, contentvec_model_path, speaker_emb_model_path):
        ...
        self.diffusion_syntheziser = DiffWave(FILTER_SIZE, RES_LAYERS, CONTENTVEC_FEAT_SIZE).to(self.device)
        ...
        self.contentvec_extractor = ContentVecExtractor(model_path=contentvec_model_path)

        self.pseudo_sp_embedder = VAE(x_dim, hidden_dim, latent_dim, self.device).to(self.device)
    ...
    def diffusion_anonymize(self, audio, speaker_emb=None):
        ...
        anonymized_audio = self.ddpm.generate(contentvec=contentvec, speaker_emb=speaker_emb, audio_len=audio.shape[-1])
    ...
    def anonymize(self, input_file, speaker_emb=None, save_intermediate=False):
        ...
        diff_anony_audio = self.diffusion_anonymize(audio=audio, speaker_emb=speaker_emb)

        # diff_anony_out_file = f"{output_file.split('/', )[0]}_diff_anony_out.wav"

        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as diff_anony_out_file:
        ...
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as cleaned_out_file:
        ...
        if save_intermediate:
            ...
            final_diff_anony_out_file = f"{output_file.split('.wav')[0]}_diff_anony_out.wav"
            final_cleaned_44khz_file = f"{output_file.split('.wav')[0]}_cleaned_44khz.wav"
...
if __name__ == "__main__":
    ...
    anonymizer = Anonymizer(diff_model_path=diff_synth_model_path, contentvec_model_path=contentvec_model_path, speaker_emb_model_path=speaker_emb_model_path)

    audio_file = '../LibriSpeech/test-clean/61/70968/61-70968-0006.flac'
    output_file = f"./generated_samples/anony/{audio_file.split('/')[-1].split('.')[0]}.wav"
