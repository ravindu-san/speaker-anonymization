...
from inference_speaker_data_loader import (
    original_dvector_loader,
    anonymized_dvector_loader,
)
        ...
        model.load_state_dict(
            torch.load(
                os.path.join(
                    ...
                    self.params["network_output_path"],
                    model_file_name,
                ),
            ...
            for speaker_name in tqdm(test_loader):
                ...
                np.save(
                    os.path.join(
                        os.path.join(
                            self.params["target_dir"],
                            self.params["dvectors_foranonym_path"],
                        ),
                        str(speaker_name) + ".npy",
                    ),
                    embeddings.mean(0),
                )
                ...
                if anonymized:
                    np.save(
                        os.path.join(
                            os.path.join(
                                self.params["target_dir"],
                                self.params["dvectors_path_anonymized"],
                            ),
                            str(speaker_name) + ".npy",
                        ),
                        embeddings,
                    ...
                    np.save(
                        os.path.join(
                            os.path.join(
                                self.params["target_dir"],
                                self.params["dvectors_path_original"],
                            ),
                            str(speaker_name) + ".npy",
                        ),
                        embeddings,
    ...
    def EER_newmethod_epochy(self, cfg_path, M=8, epochs=100):
        ...
        for _ in tqdm(range(epochs)):
            ...
            enrollment_embeddings, verification_embeddings = torch.split(
                dvector_loader, int(dvector_loader.size(1) // 2), dim=1
            )
            ...
            similarities = torch.zeros(
                (num_speakers, num_speakers * num_utterances_verification)
            )
            ...
            for i in range(num_speakers):
                for j in range(num_speakers):
                    for k in range(num_utterances_verification):
                        ...
                        similarities[
                            i, j * num_utterances_verification + k
                        ] = similarity
                        if i == j:
    ...
    def EER_newmethod_epochy_anonymized(self, cfg_path, M=8, epochs=100):
        ...
        for _ in tqdm(range(epochs)):
            ...
            (
                ...
                original_embeddings,
            ) = dvector_dataset.provide_test_anonymized_and_original()

            # Split the original embeddings into enrollment and verification sets
            enrollment_set = original_embeddings[
                :, : original_embeddings.shape[1] // 2, :
            ]
            verification_set_original = original_embeddings[
                :, original_embeddings.shape[1] // 2 :, :
            ]
            verification_set_anonymized = anonymized_embeddings[
                :, anonymized_embeddings.shape[1] // 2 :, :
            ]
