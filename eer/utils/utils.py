...
CONFIG_PATH = "/PATH/Pitch_Anonym/config/config.yaml"
...
def calc_loss_prior(sim_matrix):
    ...
    for j in range(len(sim_matrix)):
        for i in range(sim_matrix.size(1)):
            per_embedding_loss[j][i] = -(
                sim_matrix[j][i][j]
                - ((torch.exp(sim_matrix[j][i]).sum() + 1e-6).log_())
            )
...
def mfccs_and_spec(wav_file, wav_process=False, calc_mfccs=False, calc_mag_db=False):
    ...
    mel_basis = librosa.filters.mel(
        params["sr"], params["nfft"], n_mels=params["nmels"]
    )
...
if __name__ == "__main__":
    ...
    embeddings = (
        torch.tensor([[0, 1, 0], [0, 0, 1], [0, 1, 0], [0, 1, 0], [1, 0, 0], [1, 0, 0]])
        .to(torch.float)
        .reshape(3, 2, 3)
    )
