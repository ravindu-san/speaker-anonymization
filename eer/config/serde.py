...
def create_experiment(experiment_name, global_config_path):
    ...
    cfg_path = os.path.join(
        os.path.join(params["target_dir"], params["network_output_path"]), cfg_file_name
    )
...
def create_experiment_folders(params):
    try:
        path_keynames = [
            ...
            "output_data_path",
            "dvectors_path",
            "dvectors_foranonym_path",
            "dvectors_path_anonymized",
            "dvectors_path_original",
        ]
        ...
        raise Exception(
            "Experiment already exist. Please try a different experiment name"
        )
...
def open_experiment(experiment_name, global_config_path):
    ...
    cfg_path = os.path.join(
        os.path.join(
            default_params["target_dir"],
            experiment_name,
            default_params["network_output_path"],
        ),
        cfg_file_name,
    )
    params = read_config(cfg_path)
