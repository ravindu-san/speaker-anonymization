...
class LibriSpeechAnonymized(Dataset):
    ...
    def __getitem__(self, n: int) -> Tuple[Tensor, int, str, int, int, int]:
        ...
        filepath = os.path.join(
            ...
            speaker_id,
            chapter_id,
            f"{fileid_audio}{self._ext_audio}",
        )
        ...
        file_text = os.path.join(
            self._root.replace("LibriSpeechAnonymized", "LibriSpeech"),
            "test-clean",
            speaker_id,
            chapter_id,
            file_text,
...
class LibriSpeechAnonymizedDiffVC(Dataset):
    ...
    def __getitem__(self, n: int) -> Tuple[Tensor, int, str, int, int, int]:
        ...
        file_text = f"{speaker_id}-{chapter_id}{self._ext_txt}"
        file_text = os.path.join(
            self._root.replace("LibriSpeechAnonymized-DiffVC-16KHz", "LibriSpeech"),
            "test-clean",
            speaker_id,
            chapter_id,
            file_text,
...
class LibriSpeech(Dataset):
    ...
    def __getitem__(self, n: int) -> Tuple[Tensor, int, str, int, int, int]:
        ...
        file_text = f"{speaker_id}-{chapter_id}{self._ext_txt}"
        file_text = os.path.join(
            self._root, self._subset, speaker_id, chapter_id, file_text
        )
        with open(file_text) as ft:
            for line in ft:
                fileid_text, transcript = line.strip().split(" ", 1)
